...
const TIER_COST = 49;
const REQUIRED_TIER_LEVEL = 2;

const Tier2PurchaseDetails = ({ onBack }) => {
  ...
  const handlePurchase = async () => {
    ...
    if (updateError) {
      ...
      alert('Tier 2 purchase successful!');
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-2xl shadow-xl mt-10 border">
      ...
      <h2 className="text-2xl font-bold mb-1 text-gray-800">Upgrade to Tier 2 – Pro</h2>
      <p className="text-gray-600 mb-4">One-time payment of <strong>${TIER_COST}</strong> unlocks expanded trading limits and benefits.</p>
      ...
      {insufficientFunds && !alreadyUpgraded && (
        <div className="text-sm text-red-600 mb-4">
          You need to top up at least ${TIER_COST - dashBalance} to upgrade to Tier 2.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-3 text-sm text-gray-700 mb-6">
        <div className="flex items-center gap-2">
          ...
          Daily Limit: <span className="font-medium">USD 150,000</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle size={18} className="text-green-600" />
          Monthly Limit: <span className="font-medium">USD 500,000</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle size={18} className="text-green-600" />
          Lifetime Limit: <span className="font-medium">USD 2,000,000</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle size={18} className="text-green-600" />
          Coin Buy Limit: <span className="font-medium">USD 150,000</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle size={18} className="text-green-600" />
      ...
      >
        {alreadyUpgraded ? 'Already Upgraded' : `Purchase Tier 2 – $${TIER_COST}`}
      </button>
    </div>
  );
};

export default Tier2PurchaseDetails;
